...
$accordions = $widget->get_settings('cms_accordion2');
$is_new = \Elementor\Icons_Manager::is_migration_allowed();
if(!empty($accordions)) : ?>
    <div id="<?php echo esc_attr($html_id); ?>" class="cms-accordion layout3">
        <div class="inner-layout">
            <?php foreach ($accordions as $key => $value):
                ...
                $title_key = $widget->get_repeater_setting_key( 'ac_title', 'cms_accordion2', $key );
                $widget->add_render_attribute( $title_key, [
                    'class' => [ 'cms-ac-title-text' ],
                ] );
                $widget->add_inline_editing_attributes( $title_key, 'basic' );

                $content_key = $widget->get_repeater_setting_key( 'ac_content', 'cms_accordion2', $key );
            ...
            ?>
                <div class="cms-accordion-item <?php echo esc_attr($cms_animate); ?> <?php echo esc_attr($is_active?'active':''); ?>">
                    <<?php etc_print_html($title_html_tag); ?> class="cms-ac-title <?php echo esc_attr($is_active?'active':''); ?>" data-target="<?php echo esc_attr('#' . $_id.$html_id); ?>">
                        ...
                        <?php } ?>
                        <a <?php etc_print_html($widget->get_render_attribute_string( $title_key )); ?>>
