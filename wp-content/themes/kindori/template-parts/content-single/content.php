 ...
 * @package Kindori
 */
$post_tags_on = kindori_get_opt( 'post_tags_on', true );

$post_date_on = kindori_get_opt( 'post_date_on', true );

$post_navigation_on = kindori_get_opt( 'post_navigation_on', false );
$post_author_info_on = kindori_get_opt( 'post_author_info_on', false );
$post_social_share_on = kindori_get_opt( 'post_social_share_on', false );
...
<article id="post-<?php the_ID(); ?>" <?php post_class('single-hentry'); ?>>
                ...
                foreach ($gallery_list as $img_id):
                    ...
                    <div class="cms-carousel-item">
                        <a class="light-box" href="<?php echo esc_url(wp_get_attachment_image_url($img_id, 'full'));?>">
                            <img src="<?php echo esc_url(wp_get_attachment_image_url($img_id, 'post-single'));?>" alt="<?php echo esc_attr(get_post_meta( $img_id, '_wp_attachment_image_alt', true )) ?>">
                        </a>
                    </div>
                    <?php
                endforeach;
    ...
    <div class="entry-body">
        ...
        <?php if($post_social_share_on || $post_tags_on) { ?>
            <div class="entry-botton">
                <?php if($post_tags_on) :
                    kindori_entry_tagged_in();
                endif; ?>
                <?php if($post_social_share_on) {?>
                    <div class="col-box-social">
                        <label>Share: </label>
                        <?php kindori_socials_share_default();?>
                    </div>
                <?php } ?>
            </div>
        <?php } ?>
            ...
            <div class="entry-author-info">
                <div class="author-post">
                    <div class="author-avatar">
                            <?php echo get_avatar( get_the_author_meta( 'ID' ), '500x647' ); ?>
                    </div>
